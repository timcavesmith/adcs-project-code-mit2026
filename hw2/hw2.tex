...
\begin{enumerate}
	\item Write code to randomly perturb the inertia matrix of your spacecraft that you calculated in HW1. This will ensure that the solar panel normal vector is not parallel to a principle axis. A good way to do this is to calculate the eigendecomposition of your spacecraft's inertia:
	\begin{equation*}
		J = VDV^T ,
	\end{equation*}
	then add a random perturbation of e.g. a few percent to the eigenvalues in $D$:
	\begin{equation*}
		\tilde{D} = D(I + \mathrm{diag}(d)) ,
	\end{equation*}
	and also perturb the principal axis directions by sampling a random rotation of a few degrees:
	\begin{equation*}
		\tilde{V} = V \exp (\hat{v}) ,
	\end{equation*}
	where $d \in \mathbb{R}^3$ and $v \in \mathbb{R}^3$ are both sampled from Gaussian distributions with appropriate covariances. You can then multiply these back together to produce the perturbed inertia:
	\begin{equation*}
		\tilde{J} = \tilde{V}\tilde{D}\tilde{V}^T
	\end{equation*}
...
Using the simulated sensor measurements you have generated, perform static attitude estimation by solving Wahba's problem. Perform Monte Carlo simulations (i.e. many trials with randomly generated measurements) and compare the performance of at least two different algorithms. Possible metrics for comparison include accuracy and computational cost.
